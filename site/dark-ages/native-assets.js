(()=>{
  ...
  const ATLAS=new URL('assets/medieval/original-art-atlas.webp?v=4',SELF).href;
  ...
  function sprite(name,cls='',alt=''){
    ...
    if(alt){svg.setAttribute('role','img');const title=document.createElementNS(NS,'title');title.textContent=alt;svg.appendChild(title)}else svg.setAttribute('aria-hidden','true');
    const image=document.createElementNS(NS,'image');
    image.setAttribute('href',ATLAS); image.setAttributeNS(XL,'href',ATLAS);
    image.setAttribute('x','0');image.setAttribute('y','0');image.setAttribute('width',String(ATLAS_W));image.setAttribute('height',String(ATLAS_H));image.setAttribute('preserveAspectRatio','none');
    svg.appendChild(image); return svg;
  }

  function replace(el,name,alt=''){if(!el)return null;const classes=[...(el.classList||[])].join(' ');const s=sprite(name,classes,alt);if(!s)return null;el.replaceWith(s);return s}
